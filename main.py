...
# L�y GitHub username t� �ng
github_user = os.getenv("GITHUB_ACTOR", "vonga2221983")
URL = f"https://adnade.net/ptp/?user=tuanzobe&subid={github_user}"
print("URL ang m�:", URL)

TOTAL_RUN_HOURS = 6   # m�i job t�i a 6 gi�
OPEN_MINUTES = 60      # m� trang 60 ph�t
DELAY_MINUTES = 3      # ngh� 35 ph�t gi�a c�c v�ng
...
def run_loop():
    ...
    with sync_playwright() as p:
        while time.time() < end_time:
            ...
            print(f"=I M� trang: {URL}")
            page.goto(URL, wait_until="load")
            print(f"� Gi� trang m� ~{OPEN_MINUTES} ph�t...")
            human_actions(page)
            time.sleep(OPEN_MINUTES * 60)

            browser.close()
            rest = random.randint(DELAY_MINUTES, DELAY_MINUTES + 2)
            print(f"� Ngh� {rest} ph�t tr��c v�ng ti�p theo...")
            time.sleep(rest * 60)

    print(" Ho�n t�t 6 gi� ch�y  Workflow s� t� ch�y l�i n�u kh�ng c� job ci ang ch�y.")
